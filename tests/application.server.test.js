...
describe("test for root route for the api", () => {
  test("sample", async () => {
    ...
    expect(response.headers["content-type"]).toStrictEqual(expect.stringContaining("json"));
...
describe("/user/account/signup POST", () => {
  test("test for checking user account signup", async () => {
    const response = await request(application).post("/user/account/signup").send({
...
describe("/user/account/login POST", () => {
  test("test for checking user account login", async () => {
    const response = await request(application).post("/user/account/login").send({
      email: "sample@gmail.com",
      password: "ABCabc123!",
    });
