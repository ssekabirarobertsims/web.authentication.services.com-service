        ...
        await transporter.sendMail({
            ...
            html: `
               <h1>Service Login Alert</h1>
                <p>Dear ${owner.toLocaleUpperCase()}, your service for web authentication services has been able to be logged into its account successfully on ${format(new Date(), "yyyy-MM-dd")}</p>
                <strong>Service Name: ${service}</strong>
                <strong>Service Owner: ${owner}</strong>
                <strong>Service Login Date: ${format(new Date(), "yyyy-MM-dd")}</strong>
                <br />
