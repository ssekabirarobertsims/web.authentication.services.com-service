...
module.exports = async function (request, response) {
    ...
    try {
        ...
        const FoundServiceRegisteredAccounts = await pool_connection.query("SELECT * FROM users WHERE service_provider = ?", [FoundService[0][0]?.service_id]);
        const PasswordMatch = await bcrypt.compare(`${JSON.stringify(request.body?.service_password)}`, FoundService[0][0]?.service_password);
